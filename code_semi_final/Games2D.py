...
class App:
    ...
    def __init__(self, mazefile):
        ...
        self.debog = Debog()
        self.ia = IA(self.maze, self.player, self.debog)
        self.doorsopen = DoorsOpen(self.maze, self.player)
    ...
    def on_wall_collision(self):
        ...
        if not collide_index == -1:
            ...
            wall_list, obstacle_list, item_list, monster_list, door_list = self.maze.make_perception_list(self.player, self._display_surf)
            
            #self.debog.escape_wall(self.player, self.maze, wall_list, self.ia)
            #self.ia.collision_wall(wall_list)
    ...
    def on_door_collision(self):
        for door in self.maze.doorList:
            if self.player.get_rect().colliderect(door.rect):
                print("Collision avec door")
                self.doorsopen.solveDoor(self._display_surf)
                return True
        return False
